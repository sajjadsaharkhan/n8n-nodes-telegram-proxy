...
async function apiRequest(method, endpoint, body, query, option = {}) {
    ...
    // Add proxy support
    const proxyConfig = credentials.httpProxy;
...
function createSendAndWaitMessageBody(context) {
    ...
    const attributionText = 'This message was sent automatically with ';
    const link = `https://n8n.io?utm_source=${instanceId ?? 'n8n-nodes-telegram-proxy'}`;
    let text = `Please select an option:\n\n${context.getNodeParameter('message', 0)}`;
    text = `${text}\n\n_${attributionText}_[n8n](${link})`;
    const body = {
        chat_id,
        text,
        ...
        reply_markup: {
            inline_keyboard: [
                [
                    {
                        text: '✅ Approve',
                        url: context.getNodeParameter('approveUrl', 0),
                    },
                    {
                        text: '❌ Decline',
                        url: context.getNodeParameter('disapproveUrl', 0),
                    },
                ],
            ],
        },
