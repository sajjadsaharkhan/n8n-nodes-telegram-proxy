        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                if (binaryData) {
                    ...
                    const fileName = this.getNodeParameter('additionalFields.fileName', i, '');
                    const filename = fileName || itemBinaryData.fileName?.toString();
                    if (!fileName && !itemBinaryData.fileName) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `File name is needed for ${operation}. Set it in the node using the File Name parameter under Additional Fields.`);
                    }
                ...
                if (resource === 'file' && operation === 'get') {
                    if (this.getNodeParameter('download', i, false)) {
                        ...
                        const file = await GenericFunctions_1.apiRequest.call(this, 'GET', '', {}, {}, { json: false, encoding: null, uri: `${credentials.baseUrl}/file/bot${credentials.accessToken}/${filePath}`, resolveWithFullResponse: true, useStream: true });
